...
namespace EchoBot.Bot
{
    ...
    public class BotMediaStream : ObjectRootDisposable
    {
        ...
        private async void OnAudioMediaReceived(object? sender, AudioMediaReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                // });
                    ...
                    // if(participants[0].Resource.MediaStreams != null)
                    // {
                    //     participants[0].Resource.MediaStreams.ForEach(void (data) => {
                    //         Console.WriteLine($"{sender?.ToString()}Received Audio: {data.SourceId}");
                    //     });
                    // Console.WriteLine($"Received AudioMixed: {participant?.Resource?.Info?.Identity?.User?.DisplayName}");
                    // }
                }
                ...
                if (_languageService != null)
                {
                    ...
                    // Console.WriteLine($"Received AudioMixed: {this.dominantSpeaker}");
        ...
        //     participant = participants.SingleOrDefault(x => x.Resource.MediaStreams.Any(y => y.SourceId == e.CurrentDominantSpeaker.ToString()));
                    
        //     this.dominantSpeaker = participant?.Resource?.Info?.Identity?.User?.DisplayName;
        //     if(this.dominantSpeaker == null){
        //         this.dominantSpeaker = "Guest";
        //     }
        //     _logger.LogTrace($"[DominantSpeakerChangedEventArgs(DominantSpeaker={e.CurrentDominantSpeaker})]");
